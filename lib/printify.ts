...
async function fetchWithRetry(
  ...
  try {
    ...
    console.log(
      `Fetch failed, retrying in ${backoff}ms... (${retries - 1} retries left)`
    );
...
export async function getProduct(
  ...
  try {
    ...
    try {
      ...
      if (error.name === "AbortError") {
        console.error(
          `Request timed out when fetching product ${productId} from Printify`
        );
      } else {
        console.error(
          `Error fetching product ${productId} from Printify:`,
          error
        );
      }
    ...
    console.error(
      `Error in getProduct function for product ${productId}:`,
      error
    );
    // Return first mock product as fallback
    return getMockPrintifyProducts()[0];
...
export function mapPrintifyProductToLocal(product: PrintifyProduct) {
  ...
  const containsColorName = (text: string): string | null => {
    ...
    for (const color of commonColors) {
      // Check for the color as a whole word
      if (new RegExp(`\\b${color}\\b`, "i").test(lowerText)) {
  ...
  if (product.variants && Array.isArray(product.variants)) {
    product.variants.forEach((variant) => {
      if (variant.title && typeof variant.title === "string") {
        ...
        // When extracting colors from variant titles, prioritize known color names
        if (parts.length >= 2) {
          const firstPart = parts[0].trim();
          const secondPart = parts[1].trim();

          // Check if either part contains a known color name
          const firstPartColor = containsColorName(firstPart);
          const secondPartColor = containsColorName(secondPart);

          if (firstPartColor) {
            colorsSet.add(firstPartColor);
          }

          if (secondPartColor) {
            colorsSet.add(secondPartColor);
          }

          // If no color was found but we have parts, use them as potential colors
          if (!firstPartColor && !secondPartColor) {
    ...
    product.options.forEach((option) => {
      if (option.name && typeof option.name === "string") {
        ...
        if (optionName.includes("size")) {
          ...
          colorsSet.clear();
          if (option.values && Array.isArray(option.values)) {
            colorValues = option.values.filter(
  ...
  if (
    product.title &&
    product.title.toLowerCase().includes("chill") &&
    colorsSet.size === 0
  ) {
  ...
  if (colorsSet.size === 0) {
    console.log(
      "No colors found in options or variants, trying to extract from title/description"
    );
  ...
  if (colorsSet.size === 0) {
    ...
    if (
      ...
      productTags.includes("t-shirt") ||
      productTags.includes("tee")
    ) {
      colorsSet.add("Black");
      colorsSet.add("White");
      ...
      productTitle.includes("sweatshirt") ||
